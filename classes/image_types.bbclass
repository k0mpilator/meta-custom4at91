...
multiubi_mkfs() {
	...
	echo \[kernel1\] >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo mode=ubi >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo image=${IMGDEPLOYDIR}/zImage >> ubinize${vname}.cfg
	echo vol_id=1 >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo vol_type=dynamic >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo vol_name=kernel1 >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo vol_size=10MiB >> ubinize${vname}-${IMAGE_NAME}.cfg

	echo \[kernel2\] >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo mode=ubi >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo image=${IMGDEPLOYDIR}/zImage >> ubinize${vname}.cfg
	echo vol_id=2 >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo vol_type=dynamic >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo vol_name=kernel2 >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo vol_size=10MiB >> ubinize${vname}-${IMAGE_NAME}.cfg

	echo \[rootfs1\] >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo mode=ubi >> ubinize${vname}-${IMAGE_NAME}.cfg
	...
	echo vol_type=dynamic >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo vol_name=rootfs1 >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo vol_size=100MiB >> ubinize${vname}-${IMAGE_NAME}.cfg

	echo \[rootfs2\] >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo mode=ubi >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo image=${IMGDEPLOYDIR}/${IMAGE_NAME}${vname}${IMAGE_NAME_SUFFIX}.ubifs >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo vol_id=4 >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo vol_type=dynamic >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo vol_name=rootfs2 >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo vol_size=100MiB >> ubinize${vname}-${IMAGE_NAME}.cfg

	echo \[data\] >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo mode=ubi >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo image=${IMGDEPLOYDIR}/empty.ubifs >> ubinize${vname}.cfg
	echo vol_id=5 >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo vol_type=dynamic >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo vol_name=data >> ubinize${vname}-${IMAGE_NAME}.cfg
	echo vol_size=100MiB >> ubinize${vname}-${IMAGE_NAME}.cfg
    ...
    mkfs.ubifs -r ${IMGDEPLOYDIR}/empty -o ${IMGDEPLOYDIR}/empty.ubifs ${mkubifs_args}
